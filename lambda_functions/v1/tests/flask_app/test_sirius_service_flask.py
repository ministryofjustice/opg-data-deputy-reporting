...
def test_sirius_does_not_exist(
    monkeypatch, patched_submit_document_to_sirius, default_sirius_reports_request
):
    headers = {"Content-Type": "application/json"}
    body = default_sirius_reports_request

    response = sirius_service.submit_document_to_sirius(
...
def test_format_sirius_response(
    ...
    result = sirius_service.format_sirius_response(
...
def test_build_sirius_url(base_url, version, endpoint, url_params, expected_result):
    ...
    assert sirius_service.build_sirius_url(
