...
def main():
    ...
    # broker_password = get_secret("pactbroker_admin")
    broker_password = get_secret("local")
    consumer_pacticipant = "Complete the deputy report"
    provider_pacticipant = "OPG Data"
    api_version = "v1"
    git_commit_provider = "z12345"  # this will be arg
    git_commit_consumer = "x12345"  # this will be arg

    if not consumer_triggered:

        verified, message = run_pact_verifier(
            provider_base_url=provider_base_url,
            custom_header=provider_custom_header,
            pact_broker_url=pact_broker_url,
            broker_user_name=broker_user_name,
            broker_password=broker_password,
            provider=provider_pacticipant,
        ...
        if not verified and message == "Failure! No verification processed":
            ...
            verified, message = run_pact_verifier(
                ...
                broker_password=broker_password,
                provider=provider_pacticipant,
                consumer_api_version=f"{api_version}",
                git_commit_provider=git_commit_provider,
            )
        ...
        can_i_deploy, last_line = run_can_i_deploy(
            pact_broker_url=pact_broker_url,
            broker_user_name=broker_user_name,
            broker_password=broker_password,
        ...
        if (
            f"No version with tag {api_version}_production exists for {consumer_pacticipant}"
            in last_line
        ):
        ...
        if (
            f"No version with tag {api_version} exists for {consumer_pacticipant}"
            in last_line
        ):
            message, fail_build = (
    ...
    elif consumer_triggered:
        consumer_api_version = get_consumer_version(
            pact_broker_url,
        ...
        can_i_deploy, last_line = run_can_i_deploy(
            ...
            broker_password=broker_password,
            consumer_pacticipant=consumer_pacticipant,
            git_commit_provider=git_commit_consumer,
            provider_pacticipant=provider_pacticipant,
        ...
        if "There is no verified pact" in last_line:
            ...
            verify_response, verify_last_line = run_pact_verifier(
                provider_base_url=provider_base_url,
                custom_header=provider_custom_header,
                pact_broker_url=pact_broker_url,
                broker_user_name=broker_user_name,
        ...
        if (
            f"No version with tag {consumer_api_version} exists for {provider_pacticipant}"
            in last_line
        ):
            message, fail_build = (
            ...
            f"No version with tag {consumer_api_version}_production exists for {provider_pacticipant}"
            in last_line
        ):
            message, fail_build = (
...
def run_pact_verifier(
    ...
    clean_str = re.search(
        r"[0-9]+ interaction[a-z*], [0-9]+ failure[a-z*]|$", result_line
    ).group()
...
def run_can_i_deploy(
    ...
    return command_response, last_line
